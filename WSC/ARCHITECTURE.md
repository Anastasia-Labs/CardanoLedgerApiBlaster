...
| Governance (4264 B) discharges **22** `by blaster` theorems at `#prep_uplc … 9000` | `Tests/Scripts/Governance/{Governance,Properties}.lean` | Tractability precedent; seize/minting/base are smaller. **CORRECTED at task X2 — read the row below before using this one.** |
| **`proposingInputs` is a purpose DISCRIMINATOR, not a shape**, so **the entire upstream test suite is UNSHAPED** | `CardanoLedgerApi/V3/Contexts.lean:741` (`match ctx.scriptContextScriptInfo with \| .ProposingScript .. => [toTerm ctx] \| _ => [Term.Error]`); likewise `spendingInputs` (HelloWorld 10000, UnlockPIN 10000, SellNFT 1800), `mintingPolicyInputs` (1200), `oracleInputs` (10000) | **An earlier framing contrasted "Governance proves over a symbolic context" with "ours are shaped", implying a technique gap on our side. That was wrong.** `proposingInputs` freezes no list length, no constructor tag and no `Option` — it is the same class of inputs function as `WSC.baseInputs` / `WSC.mintingPolicyInputs900`. Upstream has never needed prep-shaping; **our validators are the first where unshaped does not scale.** What makes Governance close is its HYPOTHESES: `hasParamProposalWithValue` pins a `Data` **skeleton at the exact field the validator navigates** (`ppGovernanceAction = Data.Constr 0 [_, Data.Map changes, _]`) while leaving `changes`' length free — a *value* equation the Lean optimizer substitutes. Ours would have to pin a list **length**, which no hypothesis folds away (measured: same class, 4.6 s prep-shaped vs `⚠️ Undetermined` at 608 s hypothesis-shaped). Also: **there is no vacuity probe anywhere in upstream's Governance module**, so our four-point bar is strictly stronger than what the technique ships with. Calibration on this box: Governance prep @9000 = 66.5 s, 20/20 `✅ Valid` = 33 s. |
