...
* Last updated: 2026-07-25 (task Y3). Baseline commit `dd86906`.
...
| **P6** | *Claiming Member is self-penalizing: an accepted Member classification adds the positive minted amount to the value that must remain at base outputs.* | **DEFERRED** | — | Lives on the same global validator as P1: the Member-transfer accepts cost 3,262 / 3,726 CEK steps, so P6 shares P1's reachability problem unless a Member-shaped accept can be found below ~1,600 steps. None has been measured. |
...
| **P1** | *Transfers conserve programmable tokens inside the mini-ledger: for every registered `(cs, tn)`, the amount at base outputs is at least the amount at base inputs plus any positive net mint.* | **NOT-REACHABLE-AT-UPLC-see-source-model-route** | **No published K.** The containment-carrying accepts cost **3,262** and **3,726** CEK steps; preps extrapolate to ≈ 7 years and ≈ 182 years on global's own steepest MEASURED slope (K-MEASUREMENTS §5.1/§5.2). The CIP-153 `valueContains` builtin does not help: prep cost is budget-driven, not builtin-count-driven. | The B2 (algebra-assisted UPLC) route is dead over a fully symbolic context. Remaining routes: shaped contexts, or B3 (source model of the TransferAct containment) with **one** explicit `_faithful` axiom and golden cross-checks. Neither is started. |
