...
/- Z2 loosening rung: index-free P5 at SHAPE G2 (symbolic redeemer indices). -/
  ...
  ∀ (ppCS : CurrencySymbol)
    ...
    validRewardingContext
      (globalShapedCtxIdx cs tn q owner inAda dest outAda qOut pHash pCS pTn pAda pQty
        dirCS plc glc slc nHash nCS nTn nAda nQty key next tlsH ilsH gsCS w0 w1 a0 a1 fee pIdx nIdx) →
    isSuccessful
      (appliedGlobalShapedIdx1600.prop ppCS cs tn q owner inAda dest outAda qOut pHash pCS pTn pAda pQty
        dirCS plc glc slc nHash nCS nTn nAda nQty key next tlsH ilsH gsCS w0 w1 a0 a1 fee pIdx nIdx) →
      hasCoveringNode dirCS cs
        (globalShapedCtxIdx cs tn q owner inAda dest outAda qOut pHash pCS pTn pAda pQty
          dirCS plc glc slc nHash nCS nTn nAda nQty key next tlsH ilsH gsCS w0 w1 a0 a1 fee
          pIdx nIdx).scriptContextTxInfo.txInfoReferenceInputs = true

#blaster (timeout: 900) (gen-cex: 1) (solve-result: 1) [P5_G2]
...
#blaster (timeout: 900) (gen-cex: 0) (solve-result: 1) [vac_G2]
